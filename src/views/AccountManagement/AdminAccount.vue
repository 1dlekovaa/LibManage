...
const loadUsers = async () => {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      errorMessage.value = result.error?.message || 'Failed to load users'
    }
  } catch (error) {
  ...
  try {
    ...
    if (modalMode.value === 'create') {
      ...
      if (result.success) {
        ...
        modalError.value = result.error?.message || 'Failed to create user'
      }
    } else {
      // Edit mode
      if (editingUserId.value) {
        ...
        if (result.success) {
          ...
          modalError.value = result.error?.message || 'Failed to update user'
        }
      }
    }
  } catch (error) {
...
const performDelete = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      errorMessage.value = result.error?.message || 'Failed to delete user'
    }
  } catch (error) {
    errorMessage.value = 'An unexpected error occurred'
